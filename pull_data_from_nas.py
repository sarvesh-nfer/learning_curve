...
#     nas_ip = "14.140.231.202"
    ...
    for slide in slides:
        try:
            ...
            tmp_dst_path = join(dst_path, slide)
            print("tmp_dst_path: ", tmp_dst_path)
            move_slide_folder(src_path, tmp_dst_path, nas_ip,
                            transfer_type="scp",
                            user="nasdrive", reverse=True)
            other_tar = join(tmp_dst_path, "other.tar")
            dummy_path = join(tmp_dst_path, "")
            utar_data(other_tar, tmp_dst_path)

            grids = _get_grids(dummy_path)

            # for grid in grids:
            grid_src_path = join(nas_path, slide, grid, "")
            move_slide_folder(grid_src_path, tmp_dst_path, nas_ip,
                        transfer_type="scp",
                        user="nasdrive", reverse=True)
            print("dummy_path: ", dummy_path)
            delete_folder(other_tar)
        except Exception as err_msg:
            print(err_msg)
