...
# |-------------------------End of Constructor------------------------------|
    def getAoidetails(self,folder,slide_name,path_to_save):
        ...
        self.__path___ = path_to_save + "/" + "all_bestZ_plus_3linesCompleteSlide_pts" + "/" + slide_name
    ...
    def four_lines_points(self):
        ...
        if(height >= length):
            ...
            middle_start_list_vertical = [y_start + (3*length)/8]
            middle_end_list_vertical = [y_start + (5*length)/8]
        elif(length > height):
            ...
            middle_start_list_horizontal = [x_start + (3*height)/8]
            middle_end_list_horizontal = [x_start + (5*height)/8]
            
        ############## horizontal line - top, middle and bottom - points ############
        ...
        ###if squarish type grid , try to take from middle instead of complete slide
        if(length > height and (length/height) < threshold_hl):
            start_list = middle_start_list_horizontal
            end_list = middle_end_list_horizontal
            for point in range(0,len(start_list)):
                row_start = int(start_list[point])
                row_end = int(end_list[point])
                new_corner_points =  self.calculate_horizontal_points(row_start, row_end,y_start, y_end, all_grid_aoi)    
                dis_complete_slide = abs(corner_points[0][point_num-2]-corner_points[0][point_num-1]) + abs(corner_points[1][point_num-2]-corner_points[1][point_num-1])
                distance = abs(new_corner_points[0][0]-new_corner_points[0][1]) + abs(new_corner_points[1][0]-new_corner_points[1][1])
                thres = min(dis_threshold, dis_complete_slide)
                #print("dist: ", distance, "thres: ", thres)
                if(distance >= thres):
                    point_num -=2
                    corner_points[:,point_num:point_num+2] =  new_corner_points
                    point_num +=2
        ...
        if(height > length and (height/length) < threshold_hl):
            start_list = middle_start_list_vertical
            end_list = middle_end_list_vertical
            for point in range(0,len(start_list)):
                col_start = int(start_list[point])
                col_end = int(end_list[point])
                new_corner_points =  self.calculate_vertical_points(col_start, col_end,x_start, x_end, all_grid_aoi) 
        ...
        path = self.__path___
        if not os.path.exists(path): os.mkdir(path)
        complete_path = path + "/" +'6pointsCentre_triangulation_points' + '.jpeg'
        #print(complete_path)
        self.resizeImage(image,complete_path)
        # plt.imshow(image); plt.show()
...
def center_points_process(path, slide_name, NewPtsAnalysis,allpointAnalysis,count,pointStore,new_points_metric,path_to_save):
    try:
        ...
        ##searching for new points ###using 3 line points
...
#         three_points_obj.plot_plane_on_points(fit,x,y,z)
        ...
        #print("#"*10, "plot_plane_on all points : New Global Plane using new points", "#"*10, "\n")
        y = np.array(new_aoiColIdx)
        x = np.array(new_aoiRowIdx)
        z = np.array(best_z)
        three_points_obj.plot_plane_on_points(fit,x,y,z)
...
def main():
    ...
    @note Create folder by name all_bestZ_plus_3linesCompleteSlide_pts
    @note Create excel sheet by name Plane_3line_analysis.xlsx
    """
    path_to_save = "/Users/priyanka/Documents/z_index_quality"
    path = "/Users/priyanka/Documents/z_index_quality/data_new_copy/"
    excel_path = os.path.join(path_to_save,"Plane_3line_analysis.xlsx")
