...
def xtract_tar(slide_path):
    ...
    for member in my_tar.getmembers():
        if "img_pos_trial_1.jpeg" in member.name:
            print(member.name)
            my_tar.extract(member, slide_path)
            #my_tar.close()
    dst = "/home/adminspin/Music/scripts/img_pos/"+os.path.split(slide_path)[-1]
    if not os.path.exists(dst):
        os.makedirs(dst)

    shutil.copy2(slide_path+"/img_pos_trial_1.jpeg",dst)
