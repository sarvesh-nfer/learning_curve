...
df = pd.read_csv("/home/adminspin/Music/scripts/slides.csv")


path1 = "/mnt/clusterNas/dicom_data"
path2 = "/datadrive/wsi_data/xyz_shift"
user = "adminspin"
ip = "192.168.1.2"
...
for i,j in zip(df['_source.data.slide_name'],df['_source.data.cluster_name']):
    try:
        if j == "CS001":
            ...
            src_path = join(path1,i,"other.tar")
        ...
        if j == "CS003":
            ...
            command = "sshpass -p {} scp -r StrictHostKeyChecking=no {}@{}:{} {}".format("adminspin#123", user,"192.168.1.4",src_path, dst_path)
            print("**"*50,"CS001","**"*50)
            print(command)
            status = os.system(command)
        if j == "CS004":
            ...
            if not os.path.exists(dst_path):
                os.makedirs(dst_path)
            command = "sshpass -p {} scp -r StrictHostKeyChecking=no {}@{}:{} {}".format("adminspin#123", user,"192.168.1.5",src_path, dst_path)
            print("**"*50,"CS002","**"*50)
            print(command)
            status = os.system(command)
