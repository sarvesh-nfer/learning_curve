...
class MyWindow(QMainWindow):
    ...
    def sorted_time(self,path):
        searchedfiles = sorted(glob.glob(path), key=os.path.getmtime)
        return searchedfiles[-1]

    def button_clicked(self):
        
        slide_path = self.sorted_time("/home/adminspin/wsi_app/acquired_data/*")
        post_path = self.sorted_time("/home/adminspin/wsi_app/etc/post_log/*")
        print(slide_path,"\n",post_path)

        post = pd.read_csv(post_path+"/centering_check.csv")
        ...
        fig2.add_annotation(text="<b>Calibration Data for <b>"+str(glob.glob(slide_path+"/*.db")[0].split("/")[-1].split("-")[0]),xref="paper", yref="paper",showarrow=False,x=0, y=1.11,font=dict(family="Courier New, monospace",
        ...
        fig3.add_annotation(text="<b>Stack Shift Data for <b>"+str(glob.glob(slide_path+"/*.db")[0].split("/")[-1].split("-")[0]),xref="paper", yref="paper",showarrow=False,x=0, y=1.11,font=dict(family="Courier New, monospace",
                size=24,color="RebeccaPurple"))
        ...
        # conn = sqlite3.connect(glob.glob(slide_path+"/*.db")[0])
        dfk = pd.read_sql_query("select * from grid_info where grid_status = 10;",conn)

        lst = []
        for i,j in zip(dfk['grid_id'],dfk['best_row_to_start']):
        ...
        fig5.add_annotation(text="<b>Displacement Data for <b>"+str(glob.glob(slide_path+"/*.db")[0].split("/")[-1].split("-")[0]),xref="paper", yref="paper",showarrow=False,x=0, y=1.11,font=dict(family="Courier New, monospace",
                size=24,color="RebeccaPurple"))
        ...
        with open('/home/adminspin/Documents/'+str(glob.glob(slide_path+"/*.db")[0].split("/")[-1].split("-")[0])+'_report.html', 'a') as f:
            f.write(fig2.to_html(full_html=False, include_plotlyjs='cdn'))
            f.write(fig3.to_html(full_html=False, include_plotlyjs='cdn'))
            f.write(fig4.to_html(full_html=False, include_plotlyjs='cdn'))
            f.write(fig5.to_html(full_html=False, include_plotlyjs='cdn'))

        self.label.setText("Validation Completed")
        self.label.move(50,120)
        self.update()

    def initUI(self):
        self.setGeometry(200, 200, 300, 300)
        self.setWindowTitle("Scanner Validation")
