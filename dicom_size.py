...
# _get_dicom_size
# |----------------------------------------------------------------------------|
def _get_dicom_size(slide_path):
    # Read metadata_json and get grids.
    nas_dicom_path = os.path.join(slide_path,"dicom.tar")
    size_in_bytes = os.path.getsize(nas_dicom_path)
...
#     nas_ip = "14.140.231.202"
    ...
    for slide in slides:
        ...
        try:
            ...
            size = _get_dicom_size(src_path)
            print("size",size)
            sar[slide] = size
            print("sar",sar)
            df = pd.DataFrame(sar.items(),columns=['slide_name','dicom_size'])
            df2 = df2.append(df)
            ...
            file1.write("\n")
            file1.close()
    df2.to_csv("/home/adminspin/Desktop/dicom_size.csv",index=False)
