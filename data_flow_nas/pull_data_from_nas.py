    ...
    if reverse is False:
        if transfer_type == "rsync":
            ...
            command = "sshpass -p {} scp -r {} {}@{}:{}".\
                format("adminSpin#123", src_path, user, ip, dst_path)
    else:
        if transfer_type == "rsync":
            command = "sshpass -p 'adminSpin#123' rsync -e 'ssh -o "\
                "StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null' "\
            ...
            command = "sshpass -p {} scp -r {}@{}:{} {}".\
                format("adminSpin#123", user, ip, src_path, dst_path)
...
if __name__ == "__main__":
    ...
    #changes from txt to csv
    if len(sys.argv) == 2:
        file_path = sys.argv[1] #path of csv
    else:
        print("Pass a csv file path")
...
#     nas_ip = "14.140.231.202"
    ...
    for slide in df['Slide Name']:
        try:
            print(slide.strip("\n"))
            slide = slide.strip("\n")
            slide = slide.strip(" ")
            slide_path = join(dst_path, slide)
            create_folders(slide_path)
            src_path = join(nas_path, slide, "other.tar")
            dst_path = join(dst_path, slide)
            move_slide_folder(src_path, dst_path, nas_ip,
                            transfer_type="scp",
                            user="nasdrive", reverse=True)
            other_tar = join(dst_path, "other.tar")
            dummy_path = join(dst_path, "dummy")
            create_folders(dummy_path)
            utar_data(other_tar, dummy_path)

            grids = _get_grids(dummy_path)

            for grid in grids:
                grid_src_path = join(nas_path, slide, grid, "")
                move_slide_folder(grid_src_path, dst_path, nas_ip,
                            transfer_type="scp",
                            user="nasdrive", reverse=True)
