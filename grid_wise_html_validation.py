...
# df = pd.read_csv("/home/adminspin/Desktop/sarvesh/grid_outlier.csv")
################################################################################
...
def InitializeHTML(tag):
  ...
  tag = tag + "<body style=\"background:AntiqueWhite\"><font size=\"3\">Validation module wise :"
  return tag
...
def FinalizeHTML(header, r_pass, r_fail, table, htmlFile):
  ...
  header = header + "<table style=\"width:100%\" border=\"2\">\n"
  #Append the table 
  header = header + table 
  #Close the html 
  header = header +"</table>\n"
...
def PopulateResultTable(input_folder_path, output_path,tableTag):
  ...
  one = sorted(glob.glob(input_folder_path+"/*/*/BestIndexHeatMap.jpeg"))
  three = sorted(glob.glob(input_folder_path+"/*/*/fgbg_samplingPoints.jpeg"))
  four = sorted(glob.glob(input_folder_path+"/*/*/fgbg_sampledPoints.jpeg"))
  five = sorted(glob.glob(input_folder_path+"/*/*/z_translation_type_map.jpeg"))
  six = sorted(glob.glob(input_folder_path+"/*/*/focus_errorMap.jpeg"))
  seven = sorted(glob.glob(input_folder_path+"/*/*/plane_valid_index_map.jpeg"))
  print("one : ",len(one))
  ...
  # three = glob.glob(input_folder_path+"/*/*/thumbnail.jpeg")


  for i,j,k,l,m,n in zip(one,three,four,five,six,seven):
    ...
    try:
      ...
      # for blob in blobs_list:
        ...
        # bro, one min -dp
      # final_img = os.path.join(os.path.split(os.path.split(i)[0])[0])
      g = int(i.split('/')[-2].split('_')[-1])
      print(g)
      path1 = os.path.join(os.path.split(os.path.split(i)[0])[0],"loc_output_data/grid_no_"+str(g)+".jpeg")
      path2 = i
      path3 = j
      path4 = k
      path5 = l
      path6 = m
      path7 = n
      path9 = os.path.join(os.path.split(os.path.split(i)[0])[0],"loc_output_data/validInvalidGrids.jpeg")
      item = i.split('/')[-3]
      item2 = i.split('/')[-2]

      # path1 = os.path.join(os.path.split(os.path.split(i)[0])[0],"loc_output_data/finalMergedBbox.jpeg")
      tableTag = tableTag + "<th>"+str(item)+"-"+str(item2)+"</th><th>BestIndexHeatMap</th><th>Sampling</th><th>Sampled</th><th>z_translation</th><th>FEM</th><th>plane_index</th><th>validinvalid </th>"
      ...
      # tableTag = tableTag +  "<th>"+str(item)+"<br>"+str(item2)+"</th>"
      ...
      tableTag = tableTag +path1+"\" alt=\"File Not Found\" style=\"width:80%;height:20%;border:solid\"></td>\n"

      # tableTag = tableTag + "<td align=\"center\"><img src=\"file:///"
      # tableTag = tableTag +"\" alt=\"\" style=\"width:0%;height:0%;border:none\"></td>\n"

      # #Add the metric plot
      tableTag = tableTag + "<th align=\"center\"style=\"width=150\"><img src=\"file:///"
      tableTag = tableTag + path2+"\" alt=\"File Not Found\" style=\"width:80%;height:20%;border:solid\"></th>\n"

      #Add the best focused image 
      tableTag = tableTag + "<td align=\"center\"><img src=\"file:///"
      tableTag = tableTag +path3+"\" alt=\"File Not Found\" style=\"width:80%;height:20%;border:solid\"></td>\n"

      #Add the best focused image 
      tableTag = tableTag + "<td align=\"center\"><img src=\"file:///"
      tableTag = tableTag +path4+"\" alt=\"File Not Found\" style=\"width:80%;height:20%;border:solid\"></td>\n"

      # #Add the metric plot
      tableTag = tableTag + "<th align=\"center\"style=\"width=150\"><img src=\"file:///"
      tableTag = tableTag + path5+"\" alt=\"File Not Found\" style=\"width:80%;height:20%;border:solid\"></th>\n"

      #Add the best focused image 
      tableTag = tableTag + "<td align=\"center\"><img src=\"file:///"
      tableTag = tableTag +path6+"\" alt=\"File Not Found\" style=\"width:98%;height:20%;border:solid\"></td>\n"

      #Add the best focused image 
      tableTag = tableTag + "<td align=\"center\"><img src=\"file:///"
      tableTag = tableTag +path7+"\" alt=\"File Not Found\" style=\"width:80%;height:20%;border:solid\"></td>\n"

      #Add the best focused image 
      tableTag = tableTag + "<td align=\"center\"><img src=\"file:///"
      tableTag = tableTag +path9+"\" alt=\"File Not Found\" style=\"width:120%;height:40%;border:solid\"></td>\n"


      tableTag = tableTag + "<tr>\n"
      # tableTag = tableTag + "<th>"+str(item)+"</th><th>Finalmerged</th><th>FG Mask</th><th>FEM(before)</th><th>FEM(After)</th><th></th><th></th>"
