...
def direct_plot(slide_path):
    print(slide_path)
    ##extract code
    other_path = slide_path + "/other.tar"
    my_tar = tarfile.open(other_path,'r')
    for member in my_tar.getmembers():
        if not "raw_images" in member.name:
            print(member.name)
            my_tar.extract(member, slide_path)
            #my_tar.close()
            print("**"*50)
            print("Slide Successfully Extracted : ",slide_path)
            print("**"*50)
    ...
    fig5.write_image("/home/adminspin/Music/scripts/480_disp/Displacement_plot_"+slide_path.split('/')[-1]+".png")
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("/home/adminspin/Music/scripts/all_data/all_data.csv")

    for i in df['Slide Name']:
        try:
            slide_path = path+"/"+i
            direct_plot(slide_path)
